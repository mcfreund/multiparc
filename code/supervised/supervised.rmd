...
subjs.complete <- setdiff(subjs, c(subjs.missing, "197449"))
...
cv.task <- function(X, Y) {
  ...
  for (session.i in sessions) {
    ...
    for (task.train in tasks) {
      ...
      X_train <- X[, , task.train, session.i]
      y <- Y[task %in% task.train]
    
      ## make sure in same order:
      
      X_train <- scale(X_train[y$subj, ])
      y_train <- scale(y[[session.i]])
      ...
      # print(plot(fitcv, main = task.train))
      # print(range(fitcv$lambda))
